...
int main(void){
    ...
    GLFWwindow* window = glfwCreateWindow( 640, 480, "Texture Mapping T-rex", NULL, NULL );
...
void init() {
    loadJ3A("/Users/j2n2/Desktop/Ajou Univ./2022-1/CG/assignments/TextureMappingTrex/TextureMappingTrex/Archive/Trex_m.j3a");
    program.loadShaders("/Users/j2n2/Desktop/Ajou Univ./2022-1/CG/assignments/TextureMappingTrex/TextureMappingTrex/shader.vert",
                        "/Users/j2n2/Desktop/Ajou Univ./2022-1/CG/assignments/TextureMappingTrex/TextureMappingTrex/shader.frag");
    ...
    void* d = stbi_load((string("/Users/j2n2/Desktop/Ajou Univ./2022-1/CG/assignments/TextureMappingTrex/TextureMappingTrex/Archive/")+diffuseMap[0]).c_str(), &w, &h, &n, 4);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB8_ALPHA8, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, d); // 0번에 텍스쳐 올려주는 과정
    glGenerateMipmap(GL_TEXTURE_2D);
    stbi_image_free(d);

    d = stbi_load((string("/Users/j2n2/Desktop/Ajou Univ./2022-1/CG/assignments/TextureMappingTrex/TextureMappingTrex/Archive/")+bumpMap[0]).c_str(), &w, &h, &n, 4);
    printf("image : %s %d x %d\n", bumpMap[0].c_str(), w, h);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, d); // 0번에 텍스쳐 올려주는 과정
    glGenerateMipmap(GL_TEXTURE_2D);
    stbi_image_free(d);
}
...
void render( GLFWwindow* window ) {
    ...
    glActiveTexture(GL_TEXTURE0); // 0번째 slot activate
    glBindTexture(GL_TEXTURE_2D, diffTex); // diffTex Binding
    GLuint diffTexLocation = glGetUniformLocation(program.programID, "diffTex");
    glUniform1i(diffTexLocation, 0);
    
    glActiveTexture(GL_TEXTURE1); // 1번째 slot activate
    glBindTexture(GL_TEXTURE_2D, bumpTex); // bumpTex Binding
